...
def compile_rag_task_contract(
    ...
    *,
    ...
    contract = RagTaskContract(
        ...
        selected_kb_count=selected_kb_count,
        # The final execution policy, rather than the route model's declared
        # evidence scope, decides whether an implicit mapping must be carried
        # into the contract.  Local safety guards may legitimately upgrade a
        # ``general_world`` route to retrieval (for example low confidence or
        # knowledge-required input); that upgraded contract must satisfy the
        # same bridge invariant as an enterprise-scoped route.
