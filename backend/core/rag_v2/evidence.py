...
_BRIDGE_RELATION_SIGNAL_RE = re.compile(
    r"(?:对应|属于|归属|映射|适用|分类|类别|类型|等级|级别|档位|阶段|"
    r"版本|层级|序列|决定|确定)",
    re.IGNORECASE,
)
...
def _bridge_text_support(
    ...
    """Assess a bridge claim without requiring a guessed target value.

    Bridge descriptions generated by the local planner intentionally contain
    generic alternatives such as ``分类/等级/阶段``.  Requiring those exact
    words would reject valid clauses (``普通员工对应D级``), while accepting
    topical overlap alone would promote arbitrary answer chunks.  We instead
    require the left-hand entity to occur in the chunk *and* a conservative
    relation signal (or a stable identifier) to be present.
    """
    ...
    if not normalized_description or not normalized_content:
        return False, False
    match = _BRIDGE_SUBJECT_RE.search(normalized_description)
    if not match:
        return _text_support(description, content)
    subject = match.group("subject").strip(" ：:，,。；;（）()[]【】")
    subject_terms = _coverage_terms(subject)
    content_terms = _coverage_terms(normalized_content)
    if not subject_terms or not (subject_terms & content_terms):
        return False, False
    relation_signal = bool(_BRIDGE_RELATION_SIGNAL_RE.search(normalized_content))
    stable_identifier = bool(_BRIDGE_IDENTIFIER_RE.search(normalized_content))
    if not (relation_signal or stable_identifier):
        return False, False
    exact = subject and subject in normalized_content
    return True, bool(exact)


def _independent_answer_terms_match(
    answer_description: str,
    bridge_descriptions: Sequence[str],
    content: str,
) -> bool:
    ...
    for description in bridge_descriptions:
        match = _BRIDGE_SUBJECT_RE.search(
            re.sub(r"\s+", " ", str(description or "")).strip().casefold()
        )
        if match:
            bridge_subject_terms.update(_coverage_terms(match.group("subject")))
    target_terms = {
        term
    ...
    *,
    ...
    for requirement in required_answers:
        ...
        if not discriminative_terms:
            continue
        if bridge_descriptions and not _independent_answer_terms_match(
            requirement.description,
            bridge_descriptions,
            content,
        ):
...
def _reconcile_multi_hop_links(
    ...
    *,
    ...
    """Drop every answer claim that is not joined to bridge evidence."""
    ...
    # complete answer.  Once a bridge is present, however, *all* answer support
    # (including lexical/direct annotations) must join to its resolved value or
    # it is unsafe to combine.
    if not bridge_items:
        return list(items)
    ...
    for item in items:
        ...
        supports = tuple(
            requirement_id
            for requirement_id in item.supports_requirement_ids
            if requirement_id not in answer_support_ids
        )
        if not supports:
            role = "background"
        elif all(
...
def _mapped_item(
    ...
    *,
    ...
    for requirement in requirements:
        supported, exact = (
            _bridge_text_support(requirement.description, item.content)
            if requirement.role == "bridge"
    ...
    if lexical_bridge_ids:
        bridge_descriptions = [
            ...
            for requirement_id in lexical_bridge_ids
        ]
        lexical_ids = [
            requirement_id
            for requirement_id in lexical_ids
...
def assemble_evidence_bundle(
    *,
    ...
    if answer_shape == "multi_hop" and not any(
        item.role == "bridge" for item in normalized_requirements
    ):
        raise ValueError("multi_hop evidence assembly requires a bridge requirement")
