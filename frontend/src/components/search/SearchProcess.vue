    ...
    <div class="flex items-center">
      <template v-for="(step, i) in steps" :key="step.key">
        <div class="flex flex-col items-center">
          ...
          <span class="text-xs mt-1 whitespace-nowrap" :class="labelClass(step.status)">{{ step.label }}</span>
        </div>
        <div
          v-if="i < steps.length - 1"
          class="flex-1 h-0.5 mx-1 mb-5 transition-colors duration-300"
...
function stepClass(status) {
  ...
  if (status === 'active') return 'bg-blue-500 text-white ring-2 ring-blue-300 ring-offset-1'
